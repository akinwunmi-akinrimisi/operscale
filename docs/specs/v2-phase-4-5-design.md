...
- `approve/route.test.ts` (extend existing 7 tests) — add 8: paystack-init-fails-stays-founder-approved-returns-502; paystack-OK-resend-fails-flips-brief-email-failed-returns-502; both-succeed-flips-brief-sent-returns-200-with-resend-message-id; retry-on-brief-email-failed-within-1h-no-second-resend; retry-on-brief-email-failed-after-1h-sends-again; widened-order-query-includes-amount-ngn-and-tier; customer-fetch-fails-flips-brief-email-failed-returns-502 (code-review fix 2026-05-05: was missing-customer-email-returns-400, now correctly returns 502 + flips status + writes activity log); passes-paymentLink-into-rendered-template.
